...
void MFRC522::PICC_saveUID(Uid *uid){
  int count = 0;
  boolean found = false;
  for(int i=0; i<myuid_count; i++){
     count = 0;
     if (uid->size == myuid[i].size){
        for(int ii=0; ii<uid->size; ii++){
          if(uid->uidByte[ii] == myuid[i].uidByte[ii]){
            count++;
          }
        }
     }
  ...
  if(!found && myuid_count<10){
    myuid[myuid_count].size = uid->size;
    myuid[myuid_count].sak = uid->sak;
    for(int i=0; i<uid->size; i++){
       myuid[myuid_count].uidByte[i] = uid->uidByte[i];      
    }
    myuid_count++;
  }
...
void MFRC522::PICC_deleteUID(Uid *uid){
  int count = 0;
  boolean found = false;
  for(int i=0; i<myuid_count; i++){
     
     if (uid->size == myuid[i].size && !found){
        count = 0;
        for(int ii=0; ii<uid->size; ii++){
          if(uid->uidByte[ii] == myuid[i].uidByte[ii]){
            count++;
     ...
     if(found && i+1<myuid_count){
      myuid[i].size = myuid[i++].size;
      myuid[i].sak = myuid[i++].sak;
      for(int i=0; i<uid->size; i++){
        myuid[i].uidByte[i] = myuid[i++].uidByte[i];      
      }
     }
  }
